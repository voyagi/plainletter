# Art direction, round one: RETIRED

Locked and retired on 2026-08-21. Kept as a negative example, not as prior art. The live direction
is `ART-DIRECTION.md`. Nothing below governs anything.

## Why it was retired

Two reasons, both checkable rather than a matter of taste.

**It looked like the thing it exists to explain.** The live stylesheets of the services this
audience uses were read on 2026-08-21. `rijksoverheid.nl` is built on navy `#154273`, yellow
`#ffb612` and square corners, with `border-radius: 0` in twenty-two declarations. This direction
was navy `#14213D`, signal yellow `#F5C400` and square corners with heavy ink rules. A product whose
entire reason to exist is that official Dutch letters frighten people had been dressed as an
official Dutch letter. The public library where the desk physically stands looks nothing like that:
`bibliotheek.nl` runs warm near-black `#39373a` on peach `#fde5d0` with an orange accent and radii
of 8px, 16px and 30px.

**It was the fourth product in a row with the same skeleton.** Measured against
`workshop\design-registry.md`, it repeats on three of the four recorded fields: the anchor family
is a mid-century transport operations setting, the palette is one dark saturated ground plus a
signal colour plus signal accents, and the structure metaphor is an interface pretending to be a
physical board that fills one cell at a time. Only the media plan differed.

Two smaller faults follow from those. The console opened with a five-cell metric row, which is the
dashboard shape the brief explicitly ruled out. The landing page was a hero followed by a card grid
whose four frames were empty placeholder boxes.

## The text as it stood

Every page, component and piece of copy derived from this document. Its mockups and renders were
